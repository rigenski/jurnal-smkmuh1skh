                    ...
                    <tr>
                        ...
                        <td>
                            <a href="#modalEdit" data-toggle="modal"
                                onclick="$('#modalEdit #formEdit').attr('action', 'karyawan/{{$data->id}}/update'); $('#modalEdit #formEdit #username').attr('value', '{{$data->user->username}}'); $('#modalEdit #formEdit #nama').attr('value', '{{$data->nama}}');"
                                class="btn btn-warning">Edit</a>
                            <a href="#modalDelete" data-toggle="modal"
                                onclick="$('#modalDelete #formDelete').attr('action', 'karyawan/{{$data->id}}/destroy')"
...
<div class="modal fade" id="modalImport" data-backdrop="static" data-keyboard="false" tabindex="-1"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Import Excel Data Karyawan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="/admin/karyawan/import" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="data_karyawan" name="data_karyawan" accept=".xlsx, .xls">
                        <label class="custom-file-label" for="data_karyawan">Pilih File</label>
                      </div>
                      <div class="text-small text-danger mt-2">
                        * Mohon masukkan data dengan benar sebelum dikirim
                      </div>
                      <a href="/admin/karyawan/format-export" class="btn btn-warning mt-4">Unduh
                        Format
                        Import</a>
            </div>
            <div class="modal-footer">
