...
<script>
    ...
    // init data
    const setData = () => {
        const data_filtered = data.filter((value, index, self) =>
            index === self.findIndex((t) => (
                t.kelas === value.kelas
            ))
        );

        data_filtered.map((x, i) => {
        ...
        data_week.map((x, i) => {
            ...
            const data_week_filtered = x.filter((value, index, self) =>
                index === self.findIndex((t) => (
                    t.kelas === value.kelas
                ))
            );

            data_week_filtered.map((item) => {    
                const kelas = item.kelas.split(' / ');

                siswaHadirWeek[i] += parseInt(item.siswa_hadir);
                siswaTidakHadirWeek[i] += parseInt(item.siswa_tidak_hadir);

                if(kelas[0] == 'X') {
                    siswaHadirWeekX[i] += parseInt(item.siswa_hadir);
                    siswaTidakHadirWeekX[i] += parseInt(item.siswa_tidak_hadir);
                } else if(kelas[0] == 'XI') {
                    siswaHadirWeekXI[i] += parseInt(item.siswa_hadir);
                    siswaTidakHadirWeekXI[i] += parseInt(item.siswa_tidak_hadir);
                } else if(kelas[0] == 'XII') {
                    siswaHadirWeekXII[i] += parseInt(item.siswa_hadir);
                    siswaTidakHadirWeekXII[i] += parseInt(item.siswa_tidak_hadir);
                }
            })
        })
