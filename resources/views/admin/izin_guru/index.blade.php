            ...
            <div class="row w-100 mx-0">
                <div class="col-12 col-md-6 px-0">
                    <div class="d-flex justify-content-start align-items-center flex-wrap">
                        <form action="{{ route('admin.izin_guru') }}" method="get" class="d-flex align-items-center">
                            <input type="text" class="form-control datepicker mr-2 mb-2" name="tanggal" autocomplete="off"
                                value="{{ $tanggal ? $tanggal : date('Y-m-d') }}">
                            <button class="btn btn-primary mr-2 mb-2">Cari</button>
                        </form>
                    </div>
                </div>
                <div class="col-12 col-md-6 px-0">
                    <div class="d-flex justify-content-end align-items-center flex-wrap">
                        <div class="d-flex align-items-center">
                            <a href="{{ route('admin.izin_guru') }}" class="btn btn-warning px-2 ml-2 mb-2">
                            ...
                            <button type="button" class="btn btn-info ml-2 mb-2" data-toggle="modal"
                                data-target="#modalSetting">
                                Pengaturan
                            </button>
                            <a href="{{ route('admin.izin_guru.export') }}" class="btn btn-success ml-2 mb-2">Export</a>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                @foreach ($data_kelas as $kelas)
                                    ...
                                    @foreach ($data_jurnal as $jurnal)
                                        @if ($jurnal->kelas == $kelas->kelas)
                                            ...
                                            @if (count($jam_ke) == 2)
                                                ...
                                                @for ($i = $jam_ke[0]; $i <= $jam_ke[1]; $i++)
                                                    <?php array_push($jam_arr, $i); ?>
                                                @endfor
                                    ...
                                    @if (count($data_pengaturan_izin_guru))
                                        ...
                                        <?php $pengaturan_jam = count($data_pengaturan_izin_guru)
                                            ? explode(
                                                '-',
                                                $data_pengaturan_izin_guru
                                                    ->where('hari', $hari)
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.izin_guru.setting') }}" method="post">
                        @csrf
                        <div id="accordion">
                                        ...
                                        @foreach ($data_kelas as $kelas)
                                            <div class="form-group row mb-1">
                                                ...
                                                <div class="col-sm-9 row px-0">
                                                    <?php $jam_ke = count($data_pengaturan_izin_guru)
                                                        ? explode(
                                                            '-',
                                                            $data_pengaturan_izin_guru
                                                                ->where('hari', $hari)
                                                    ...
                                                    <div class="col-6 px-0 pr-1">
                                                        <select class="form-control-sm bg-white w-100"
                                                            ...
                                                            name={{ $hari . '-' . str_replace(' ', '_', $kelas->kelas) . '-' . 'jam_ke' }}>
                                                            @foreach ($data_jam as $jam)
                                                                @if (count($data_pengaturan_izin_guru))
                                                                    @if ($jam == $jam_ke[0])
                                                    ...
                                                    <div class="col-6 px-0 pl-1">
                                                        <select class="form-control-sm bg-white w-100"
                                                            style="border: 1px solid #ced4da;"
                                                            name={{ $hari . '-' . str_replace(' ', '_', $kelas->kelas) . '-' . 'sampai_jam_ke' }}>
                                                            @foreach ($data_jam as $jam)
                                                                @if (count($data_pengaturan_izin_guru))
                                                                    @if ($jam == $jam_ke[1])
                                                                        <option selected>{{ $jam }}</option>
